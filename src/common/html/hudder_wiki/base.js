document.addEventListener("DOMContentLoaded", function (event) {
	...
	for (var i = 0; i < notes.length; i++) {
		...
		notes[i].innerHTML = "<img src=\"Images/Info.webp\" alt=\"INFO\"><span>" + content + "</span>";
		notes[i].insertAdjacentElement("afterend", document.createElement("br"));
	}
	var notes = document.getElementsByTagName('warning');
	for (var i = 0; i < notes.length; i++) {
		let content = notes[i].innerHTML;
		notes[i].innerHTML = "<img src=\"Images/Warning.png\" alt=\"WARNING\"><span>" + content + "</span>";
		notes[i].insertAdjacentElement("afterend", document.createElement("br"));
	}
	var notes = document.getElementsByTagName('deprecatedwarning');
	for (var i = 0; i < notes.length; i++) {
		let content = notes[i].innerHTML;
		notes[i].innerHTML = "<img src=\"Images/Deprecated.png\" alt=\"DEPRECATED\"><span>" + content + "</span>";
		notes[i].insertAdjacentElement("afterend", document.createElement("br"));
	}
